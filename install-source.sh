...
#   curl -fsSL PRISM_SRC_DIR="$PWD" bash install-source.sh
## @file
...
BRANCH="master"
SRC_DIR="${PRISM_SRC_DIR:-${XDG_CACHE_HOME:-$HOME/.cache}/prism/source}"
BUILD_JOBS="$(nproc)"
...
sudo dnf install -y \
  ...
  kscreen krfb labwc wlr-randr xrandr wayland-utils xorg-x11-server-Xwayland \
