...
%global branch 0
%global commit 0
%global prism_python_version 3.14
...
mkdir -p %{_builddir}/Prism
tar -xzf %{SOURCE0} -C %{_builddir}/Prism

# list directory
ls -a %{_builddir}/Prism
...
cmake_args=(
  ...
  "-DPRISM_ASSETS_DIR=%{_datadir}/prism"
  "-DPRISM_EXECUTABLE_PATH=%{_bindir}/prism"
...
cmake_args+=("-DPython_EXECUTABLE=%{_builddir}/Prism/.venv/bin/python")
%endif
...
function install_cuda() {
  ...
  if [ -n "${patch_file}" ]; then
    ...
    patch -p2 \
      ...
      --directory="%{cuda_dir}" \
      --verbose \
      < "%{_builddir}/Prism/packaging/linux/patches/${architecture}/${patch_file}"
  fi
}
...
%if 0%{?sle_version}
cmake_args+=("-DPRISM_ENABLE_VULKAN=OFF")
%endif

# cmake
cd %{_builddir}/Prism
%if 0%{?fedora}
uv python install %{prism_python_version}
uv sync \
  --locked \
  --only-group glad \
  --python %{prism_python_version} \
  --no-install-project
%endif
echo "cmake args:"
echo "${cmake_args[@]}"
cmake "${cmake_args[@]}"
make -j$(nproc) -C "%{_builddir}/Prism/build"
...
# run tests
cd %{_builddir}/Prism/build
xvfb-run ./tests/test_prism
...
%files
# Executables
%caps(cap_sys_admin,cap_sys_nice+p) %{_bindir}/prism

# Systemd unit files for user services
%{_userunitdir}/*.service

# Udev rules
%{_udevrulesdir}/*-prism.rules

# Modules-load configuration
%{_modulesloaddir}/*-prism.conf

# Desktop entries
%{_datadir}/applications/*.desktop

# Icons
%{_datadir}/icons/hicolor/scalable/apps/*.Prism.svg
