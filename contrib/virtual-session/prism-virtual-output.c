...
static void stream_serial(void *data, struct zkde_screencast_stream_unstable_v1 *stream, uint32_t hi, uint32_t low) {
  (void) data;
  (void) stream;
...
static void registry_global(void *data, struct wl_registry *registry, uint32_t name, const char *interface, uint32_t version) {
  ...
  if (strcmp(interface, zkde_screencast_unstable_v1_interface.name) == 0) {
    ...
    g.screencast = wl_registry_bind(registry, name, &zkde_screencast_unstable_v1_interface, g.screencast_version);
  }
}
