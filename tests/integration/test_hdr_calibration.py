#!/usr/bin/env python3
"""@brief Test HDR wizard input, preview, save and cancel on a GPU."""
import os
...
def run_test(build, compositor):
    """@brief Run GPU checks separately from desktop and streaming sessions."""
    with tempfile.TemporaryDirectory(prefix="prism-hdr-wizard-") as temporary:
        root = Path(temporary)
        config = root / "labwc"
        config.mkdir()
        (config / "rc.xml").write_text(
            "<labwc_config><core><hdr>yes</hdr>"
            "<adaptiveSync>yes</adaptiveSync></core></labwc_config>"
        )
        ...
        with (root / "log").open("w") as log:
            try:
                server = subprocess.Popen(
                    [str(compositor), "--config-dir", str(config)],
                    env=environment,
                ...
                for _ in range(80):
                    sockets = [
                        entry for entry in root.glob("wayland-*")
                        if entry.is_socket()
                    ]
                    if sockets:
                        break
                ...
                def launch():
                    """@brief Start the wizard and wait for its first frame."""
                    child = subprocess.Popen(
                        [str(build / "prism-hdr-calibration")],
                        env=environment,
                        stdout=log, stderr=log, start_new_session=True,
                    )
                ...
                def keys(*codes):
                    """@brief Send controller-equivalent keys over Wayland."""
                    subprocess.run(
                        [str(build / "hdr-test-keyboard"), *map(str, codes)],
                        env=environment, check=True, timeout=5,
                    )
                    time.sleep(0.3)

                def white_pixel():
                    """@brief Sample the PQ patch via compositor screencopy."""
                    ...
                    index = header.end() + (
                        int(height * 300 / 720) * width
                        + int(width * 520 / 1280)
                    ) * 3
                    return image[index]

                app = launch()
                assert abs(white_pixel() - 148) <= 1, (
                    "203-nit white was not preserved through HDR composition"
                )
                keys(106)  # Right: raise SDR white to 213 nits.
                assert live.read_text() == "1 213 1000\n"
                assert not profile.exists(), (
                    "preview modified the persistent profile"
                )
                actual = white_pixel()
                ...
                # Failed saves stay in the wizard without claiming success.
                profile.unlink()
                profile.mkdir()
                ...
                assert app.wait(timeout=5) == 0
                print(
                    "PASS: HDR pixel values, live preview, controller "
                    "navigation, per-device save, cancel and failed save"
                )
            except Exception:
                log.flush()
